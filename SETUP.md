...
- 親ページ配下に「task-manager 設定」ページを作成
- 案件DB / タレントDB / クライアントDB / メタ情報DB を作成
- 設定ページに運用ルールを書き込み
- 動作テスト（ダミー案件で Formula 検証）

完了すると 4 つの DB の URL が表示されます。Notion で開いて構造を確認してください。
...
`/tm-setup` 完了時に表示された 4 DB の data_source_id を控えておきます。Notion の「task-manager 設定」ページ配下のメタ情報DB から確認できます。
...
### 7-3. 初回手動実行で承認を済ませる（重要）

Claude Desktop の **「Scheduled」セクション**（サイドバー）から `tm-sync-daily` を選び、**「Run now」** をクリック。

- 初回実行時に Gmail / Notion コネクタの権限承認ダイアログが出る
- 一度承認すれば以降の定期実行は自動で進む
- これをやらないと、自動実行のたびに「承認待ち」で止まります
