...
- **v1.0.10** (2026-07-14): ダッシュボード最上部に「🕐 最終更新: 日時」を表示（AI が最後に Notion を更新した時刻）。データ源はメタ情報DBの最終sync日時（既存・追加処理なし）。tm-sync が手順6.5でこの行を毎回置換（前回 LAST_SYNC 値で old_str をマッチ、形式は `YYYY-MM-DD HH:MM` 固定）。tm-setup のダッシュボードテンプレにも行を追加。マッチ失敗時は insert_content でフォールバック
- **v1.0.9** (2026-07-14): 運用先の要望3点 + クライアントカルテを追加。(1) 新プロパティ4つ: 返信期日（人間入力）/ 重要度（人間入力、🔥高・中・低）/ 最終受信日（AI入力）/ こちらの放置日数（`now()` ベースの Formula で毎日自動計算 — AI の日次書き込み不要、実機検証済み）。(2) ダッシュボードに ⭐重要案件 枠を追加（重要度=🔥高のみ FILTER）、🔥/⏳ の表示列に返信期日・放置日数を反映。(3) クライアントカルテ: クライアントDB の行ページ本文に「やり取りサマリ + フィルタ済みライブ案件ビュー + マイルストーン統合」を実装。DB行ページへのビュー埋め込みが可能なことを実機確認。tm-add-client がカルテ雛形を自動生成、tm-sync がマイルストーン検知時にサマリを更新（蒸留済みAIメモから生成、Gmail 再読なし）。返信期日・重要度は AI 不可侵の人間専用フィールドと明記
